	...
	/* get existing user record */
   	...
   	/* if new user... */
    	if ( $thisuser === false )
          ...
          /* Create a custom user login ID from the email address */

          // Step 1: urlencode the user_email, then replace "%" (which is invalid
          // sign in Wordpress id) by "_"
          $urlencoded_email = str_replace("%", "_", urlencode($user_email));

          // Step 2: split the encoded email ("_40" is the encoded "@")
          $parts = explode("_40", $urlencoded_email);

          // Step 3: use the first part of the email as the user login
          $user_login = $parts[0];
